...
namespace CodeKnowledgeGraphGenerator
{
    public class CodeAnalyzer
    {
        ...
        public async Task AnalyzeAsync()
        {
            ...
            Console.WriteLine($"Analysis complete. Found {_entities.Count} entities and {_relationships.Count} relationships.");
        }

        private void ExtractEntities(SyntaxNode root, SemanticModel semanticModel, string projectName)
        {
            // Extract classes
        ...
        private void ExtractRelationships(SyntaxNode root, SemanticModel semanticModel, string projectName)
        {
            ...
            foreach (var classDeclaration in root.DescendantNodes().OfType<ClassDeclarationSyntax>())
            {
                ...
                if (symbol.BaseType != null && !symbol.BaseType.Name.Equals("Object"))
                {
                    ...
                    _relationships.Add(new CodeRelationship
                    {
                        SourceId = classId,
                        TargetId = baseTypeId,
                        Type = "Inherits"
